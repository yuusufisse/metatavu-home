...
const getVacationRequestStatuses = () => {
  ...
  const [ latestVacationRequestStatuses, setLatestVacationRequestStatuses ] = useState<VacationRequestStatus[]>([]);
  ...
  const fetchVacationRequestStatuses = async () => {
    if (vacationRequests) {
      try {
        ...
        await Promise.all(vacationRequests.map(async vacationRequest => {
          let createdStatuses: VacationRequestStatus[] = [];
          if (vacationRequest.id) {
            createdStatuses = await vacationRequestStatusApi.listVacationRequestStatuses({ id: vacationRequest.id });
          }
