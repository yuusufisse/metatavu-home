...
import { getProjectColor, getProjectName, getTotalTimeEntriesAllocations, timeLeft, totalAllocations } from "src/utils/sprint-utils";
...
const sprintViewProjectsColumns = ({allocations, timeEntries, projects}: Props) => {
  ...
  const columns: GridColDef[] = [
    ...
    { 
      ...
      flex: 1, 
      valueGetter: (params) => getHoursAndMinutes(getTotalTimeEntriesAllocations(params.row, allocations, timeEntries))
    },
    { 
      ...
      flex: 1, 
      cellClassName: (params) => timeLeft(params.row, allocations, timeEntries) < 0 ? "negative-value" : "", 
